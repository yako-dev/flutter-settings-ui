...
Brightness: `SettingsList.brightness` if set; otherwise `applicationType` picks `Theme` (`material`), `CupertinoTheme` (`cupertino`), or, for `both`, `CupertinoTheme` when the app runs on iOS/macOS and `Theme` elsewhere (by the detected platform, not the `platform` override). A forced brightness works in all six styles. Android and web build their colors from the app's `ColorScheme`, so when it has the other brightness they use a scheme of the forced one seeded from its primary color (`ThemeProvider.colorSchemeOf`) and put it in a `Theme` over their sections and tiles (`ThemeProvider.withListColorScheme`); the iOS switch takes the list's brightness (`SettingsStyleScope.brightnessOf`), not the app's.
...
- Each pane is its own semantics container: the detail navigator's routes block the semantics of what was painted before them in their container. The macOS, Windows and GNOME sidebar rows are semantics containers that carry their own selected flag (`semanticsSelected`); an annotation around them would land on their section's node.
...
- **Split view** (measured on iPadOS 27, Android 16 two-pane Settings and Chrome): iPad 320pt sidebar on #E2E6F0/#181D20, 288x52 capsule selection #0080F5/#13A4FF, no divider, pages fill the pane with 20pt margins; Android list pane 36.36% on `surfaceDim` with the phone cards, the selected card in `surfaceContainer`, no icons under 380dp, 36sp collapsing page titles at pane + 24dp; Chrome 266px menu with 40px items and an end-rounded pill, the 680px column placed like Chrome's (`SettingsContentColumnHint`). Desktop styles keep their pages' columns and margins in the detail pane, where only iOS and Android pages fill it:
  - macOS (System Settings, macOS 27; notes and a SwiftUI replica): 232pt full-height sidebar #EDEDED/#282828 with a 0.5pt edge, 32pt rows, selection radius 8 inset 10 in the accent (#0070F5/#007AFF) with a white semibold label, #D7D7D7/#464646 when the app is inactive; 20pt icons (plain icons tinted #0077FF/#008FFF), 11pt bold #A0A0A0/#5A5A5A headers, 13pt between sections, no hover; 52pt toolbar with a 73x36 back/forward capsule (forward always disabled) and a 15pt semibold #4C4C4C/#E9E9E9 title. Two panes from 560 (232 + 328 detail; SwiftUI never collapses on the Mac, but the style also runs on phones and the web).
  - Windows (WinUI `NavigationView`, measured on Windows 11 Settings): expanded from 1008 (300 wide: items 280 wide, 16 from the edge), 48px compact rail from 641 with tooltips and a menu button (the overlay pane is 320), one pane below; items >= 36 tall, 4/2 margins, radius 4, 16px icon in a 40 column, #EAEAEA/#2D2D2D selected, hover = selected fill, pressed #EDEDED/#292929; 3x16 radius-2 accent pill that stretches from the old item to the new one (350ms); pane and page share the Mica fallback color; 28/36 semibold page title, breadcrumb with chevrons for nested pages. Title and breadcrumb stay on one line like WinUI's `BreadcrumbBar`: the leading crumbs collapse into a "…" crumb that goes back to the last collapsed page, and a title too long for the line ends in an ellipsis.
  - GNOME (`AdwNavigationSplitView`, libadwaita 1.10): sidebar 25% clamped 180-280sp, collapses at 550sp; #EBEBED/#2E2E32 with a 1px border; 46px header bars with centered bold titles; 43px rows 2 apart, 6px margins, radius 9, 16px icons; hover 7%, selected 10%, selected+hover 13%, pressed 16% of the foreground; 1px 15% separators with 6px margins; 2px accent 50% focus ring; flat 34x34 back button when collapsed or nested.
