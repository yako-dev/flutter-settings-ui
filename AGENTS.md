...
Layout: `SettingsList` sizes its default padding from its own width (`LayoutBuilder`), keeping content in an 810 column (680 on web; 640 on macOS; 1000 with 36 margins on Windows; on Linux an `AdwClamp`-style column that eases from 400 to at most 600, in sp), centered or at the start edge (`crossAxisAlignment: start`). `contentPadding` replaces all of that. An empty `SettingsSection` renders `SizedBox.shrink()`.
