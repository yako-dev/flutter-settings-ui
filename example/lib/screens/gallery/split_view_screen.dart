...
///
/// Open it from the gallery, or directly with an initial route, e.g.
/// `flutter run --route '/split-view?style=android&page=display'`
/// (see `main.dart`).
class SplitViewScreen extends StatefulWidget {
  ...
  /// Reads `style` (ios, android, web, macos...) and `page` (a destination
  /// id) from the query of an initial route.
  factory SplitViewScreen.fromQuery(Map<String, String> query) {
    final style = query['style'];
    final platform = DevicePlatform.values.firstWhere(
      (value) => value.name.toLowerCase() == style?.toLowerCase(),
      orElse: () => DevicePlatform.device,
    );
    return SplitViewScreen(platform: platform, initialPageId: query['page']);
  }
