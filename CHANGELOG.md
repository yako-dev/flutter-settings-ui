...
* New default look that matches iOS 26/27, Android 16/17, Windows 11 and current Chrome settings (see Design refresh). Sizes, paddings and default colors changed. Colors you set in `SettingsThemeData` still win.
* iOS-style switch tiles (iOS, macOS) now show the new `CupertinoSettingsSwitch` instead of `CupertinoSwitch`, and Windows switch tiles the new `FluentSettingsSwitch`. Tests that find `CupertinoSwitch` need to look for `CupertinoSettingsSwitch` (or `FluentSettingsSwitch` on Windows).
