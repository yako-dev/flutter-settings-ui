...
* `SettingsSplitView` on the desktop styles: each draws its settings app's sidebar and page header.
  ...
  * GNOME: the GNOME Settings sidebar (a quarter of the window, 180 to 280, collapsing at 550; 43px rounded rows; its own header bar) and flat header bars with centered titles and a back button when collapsed or nested. In one pane the sidebar fills the window.
